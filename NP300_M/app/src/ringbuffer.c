...
rt_inline rt_ringbuffer_state_e rt_ringbuffer_status(rt_ringbuffer_t *rb)
{
...
{
	rt_ringbuffer_state_e old_state;
...
rt_ringbuffer_t * rt_ringbuffer_create(rt_uint16_t size)
{
	...
	rb = (rt_ringbuffer_t *)rt_malloc(sizeof(rt_ringbuffer_t));
	if (rb == RT_NULL){
			goto exit;
	}
